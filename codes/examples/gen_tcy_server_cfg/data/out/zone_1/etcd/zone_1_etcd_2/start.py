...
with IndentFlag():
    ...
    run_cmd = "docker run {opt} --name {name} {network} {ip} {mount_volumes} {image} {command}".format(
        ...
        command=r"etcd --name zone_1_etcd_2 --data-dir /root/zone/zone_1/etcd/zone_1_etcd_2 --listen-peer-urls http://0.0.0.0:2380 --listen-client-urls http://0.0.0.0:2379 --initial-advertise-peer-urls http://10.0.1.187:2380 --advertise-client-urls http://10.0.1.187:2379  --log-output stdout --initial-cluster-token 'zone_1' --initial-cluster zone_1_etcd_1=http://10.0.1.186:2380,zone_1_etcd_2=http://10.0.1.187:2380,zone_1_etcd_3=http://10.0.1.188:2380"
    )
    ret, out_txt, error_txt = paramiko_ssh_cmd(ssh_client, run_cmd)
    if 0 != ret:
