...
with IndentFlag():
    ...
    run_cmd = "docker run {opt} --name {name} {network} {ip} {mount_volumes} {p_ports} {image} {command}".format(
        ...
        command=r"/root/build/service world_sentinel /root/zone/zone_1/game_server/zone_1_world_sentinel_0 /root/zone/zone_1/game_server/zone_1_world_sentinel_0 /root/zone/zone_1/game_server/zone_1_world_sentinel_0/game_config.xml /root/code/tanchiyu/server/lua_script --lua_args_begin-- -lua_path /root/code/tanchiyu/server/lua_script . -c_path . -require_files servers.entrance.server_entrance  -execute_fns start_script", p_ports=" ".join(opt_publish_ports)
    )
