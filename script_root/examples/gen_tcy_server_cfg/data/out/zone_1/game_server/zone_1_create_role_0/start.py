...
with IndentFlag():
    cmds = []
    cmds.append("docker container kill {name}".format(name="zone_1_create_role_0"))
...
with IndentFlag():
    ...
    ret, out_txt, error_txt = paramiko_ssh_cmd(ssh_client, "docker exec {name} {command}".format(name=ct_name, command=''' mkdir -p /root/zone/zone_1/game_server/zone_1_create_role_0 '''))
    if 0 != ret:
        print("docker exec: run cmd fail, exit_code is {0}\nstd_out is {1}\nstd_error is {2}\n-------------\n".format(ret, out_txt, error_txt))
    ret, out_txt, error_txt = paramiko_ssh_cmd(ssh_client, "docker exec {name} {command}".format(name=ct_name, command=''' mkdir -p /root/zone/zone_1/game_server/zone_1_create_role_0/hotfix_dir '''))
    if 0 != ret:
        print("docker exec: run cmd fail, exit_code is {0}\nstd_out is {1}\nstd_error is {2}\n-------------\n".format(ret, out_txt, error_txt))
...
paramiko_sftp_put(ssh_client, 'D:/work_space/code/lxl-python/script_root/examples/gen_tcy_server_cfg/data/out/zone_1/game_server/zone_1_create_role_0/game_config.xml', "/tmp/docker_put_to_data_{0}".format(1))
with IndentFlag():
    # run docker container
    import random
    ct_name = "ct_{}".format(random.randint(1, 99999999))
    ...
    if 0 != ret:
        ...
        sys.exit(ret)
    # execute cmds in docker contianer
    ret, out_txt, error_txt = paramiko_ssh_cmd(ssh_client, "docker exec {name} {command}".format(name=ct_name, command=''' mkdir -p `dirname /root/zone/zone_1/game_server/zone_1_create_role_0/game_config.xml` '''))
    if 0 != ret:
        print("docker exec: run cmd fail, exit_code is {0}\nstd_out is {1}\nstd_error is {2}\n-------------\n".format(ret, out_txt, error_txt))
    ret, out_txt, error_txt = paramiko_ssh_cmd(ssh_client, "docker exec {name} {command}".format(name=ct_name, command=''' cp -f /root/tmp/docker_put_to_data_1 /root/zone/zone_1/game_server/zone_1_create_role_0/game_config.xml  '''))
    if 0 != ret:
        print("docker exec: run cmd fail, exit_code is {0}\nstd_out is {1}\nstd_error is {2}\n-------------\n".format(ret, out_txt, error_txt))
    # remove docker container
    paramiko_ssh_cmd(ssh_client, [
...
with IndentFlag():
    ...
    run_cmd = "docker run {opt} --name {name} {network} {ip} {mount_volumes} {p_ports} {image} {command}".format(
        opt="-d", name="zone_1_create_role_0", network=opt_network, ip=opt_ip, mount_volumes=" ".join(opt_mount_volumes), image="lxl_debian",
        command=r"/root/build/service create_role /root/zone/zone_1/game_server/zone_1_create_role_0 /root/code/tanchiyu/server/datas /root/zone/zone_1/game_server/zone_1_create_role_0/game_config.xml /root/code/tanchiyu/server/lua_script --lua_args_begin-- -lua_path /root/code/tanchiyu/server/lua_script . -c_path . /root/build -require_files servers.entrance.server_entrance  -execute_fns start_script", p_ports=" ".join(opt_publish_ports)
    )
